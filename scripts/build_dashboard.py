...
<section class="grid">
{{PROVISIONAL_BANNER}}
<article class="panel full" id="today" style="border-color:var(--accent)"><h2 style="color:var(--accent)">今天要的進出 · {{PLANNED_DATE}}</h2><div class="sub">你的策略卡對這個交易日開出的動作，只列<b>還需要決定</b>的：「出」只列仍持有的，「進」只列整戶零部位的，括號部位依指示排除。這裡只回報系統說了什麼、帳戶做了什麼 —— 要不要執行、幾塊錢，是你的決定。完整的價格分布與掛單落點在 <a href="prep/">備戰頁</a>；已經變現的盈虧全部搬到 <a href="realized/">已實現頁</a>；每一檔持股的均線、波段低點與各價位了結損益在 <a href="positions/">持股體檢</a>。</div><div class="table-wrap"><table><thead><tr><th>訊號日</th><th>策略</th><th>股票</th><th>動作</th><th>生效日</th><th>目前狀態</th></tr></thead><tbody>{{UNEXECUTED_SIGNALS}}</tbody></table></div></article>
{{OVERDUE_EXIT_COST}}
...
<footer class="footer"><span><a href="realized/" style="color:var(--green);text-decoration:none">已實現頁 已變現盈虧 →</a> · <a href="prep/" style="color:var(--green);text-decoration:none">備戰頁 下一交易日動作量測 →</a> · <a href="mainline2/" style="color:var(--green);text-decoration:none">主線二 未持有訊號追蹤 →</a> · <a href="claude/" style="color:var(--green);text-decoration:none">Claude 版精進盤點 →</a> · <a href="positions/" style="color:var(--green);text-decoration:none">持股體檢 每檔位置 →</a> · <a href="history/" style="color:var(--green);text-decoration:none">歷史存檔 每日凍結版 →</a> · 口徑：252 trading days · rf=0 · CAGR 365.25 calendar days · Alpha=daily OLS intercept×252</span><span>生成時間：<span class="mono">{{GENERATED_AT}}</span></span></footer>
</main><script>
(function () {
